...
from utils import get_image_arr, get_slice
...
def get_disparity_map(left_img_arr, right_img_arr, edge_offset):
    ...
    for row in range(edge_offset, num_valid_rows):
        print(f"Processing row {row}/{num_valid_rows}")
        ...
        while (len(columns) > 0):
            ...
            for potential_matching_col in range(current_column - 1,
                                                max(current_column - ASSUMED_MAX_DISPARITY, edge_offset - 1), -1):
