...
def get_smoothed(distance_map, region_map, clip_threshold):
    ...
    for row in range(OFFSET, distance_map.shape[0]):
        for col in range(OFFSET, distance_map.shape[1]):
            ...
            if Z == np.Inf or Z == np.NINF or Z == np.NaN or Z < 0:
                continue
                ...
                for j in range(-4, 5):
                    ...
                    if 0 <= surrounding_pixel_row < distance_map.shape[0]:
                        if 0 <= surrounding_pixel_col < distance_map.shape[1]:
                            ...
                            # python evaluates each integer in the array, so we need to check if all are true.
                            is_region_related = region_map[row, col] == region_map[
                                surrounding_pixel_row, surrounding_pixel_col]

                            if (is_distance_related and is_region_related.all()) or Z > clip_threshold:
                                num += 1
                                s += thisZ
